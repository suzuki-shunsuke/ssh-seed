...
const CMD_SSH_KEYGEN = path.join(__dirname, '../sh/expect-ssh-keygen');
const CMD_SSH_ADD = path.join(__dirname, '../sh/expect-ssh-add');

exp.sshKeyGen = thunkify((file, passphrase, callback) => {
  exec(`${CMD_SSH_KEYGEN} ${file} ${passphrase}`, callback);
});

exp.sshAdd = thunkify((file, passphrase, callback) => {
  exec(`${CMD_SSH_ADD} ${file} ${passphrase}`, callback);
});
...
exp.setDefaultConf = config => {
  ...
  const conf = {};
  ['pwgen', 'ssh_add', 'ssh_keygen'].forEach(key => {
